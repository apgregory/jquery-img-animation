;(function(factory) {
    ...
    Flipbook.prototype.nest = function() {
        var _ = this;
        _.$initialFrame.addClass('initial-frame');
        _.$initialFrame.wrap('<span class="flipbook-flipper"></span>');
    ...
    Flipbook.prototype.animate = function() {
        ...
        _.preload(imageSources, function() {
            for (var i = 0; i < imageSources.length; i++) {
                _.$flipper.append('<img class="flipper-frame" src="' + imageSources[i] + '">');
            }
    ...
    Flipbook.prototype.getImageSources = function(pre, post){
        var _ = this;
        var initialFrameSource = _.$initialFrame.attr('src').split('?');
        var imgSrcPre = initialFrameSource[0];
        var imgSrcPost = initialFrameSource[1];
        var imageSources = [];
        // if (imgSrcPre.slice(-1) === '0') imgSrcPre = imgSrcPre.slice(0, -1);
        for (var i = 1; i < _.options.frames; i++) {
            imageSources.push(imgSrcPre + '-' + i + '?' + imgSrcPost);
        }
        return imageSources;
    };
    ...
    Flipbook.prototype.flip = function(context){
        var _ = context;
        var $activeFrame = _.$flipper.find('.initial-frame, .active-frame').eq(-1);
